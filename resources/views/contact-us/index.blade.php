<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-600">Contact with experts</p>
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-3 border-b border-slate-200 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h3 class="text-lg font-semibold text-slate-900">Recent Contact</h3>
                <p class="mt-1 text-sm text-slate-500">Showing contact requests submitted by learners and customers.</p>
            </div>
        ...
        @if($details->isEmpty())
            <div class="px-6 py-12 text-center text-slate-500">
                No Contact with experts found yet.
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @foreach($details as $detail)
                            ...
                            <tr class="hover:bg-slate-50 @if($detail->status == 'new') font-black text-8xl text-emerald-600 @endif">
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{{ $detail->subject }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{{ $detail->message }}</td>
